...
consumer run the optional LLM repair path with
`llm_tasks=("display_translation",)` and a stable cache directory. This keeps the
spend decision in the product while keeping Google Maps-specific translation
memory and prompt behavior in `gmaps-scraper`.
